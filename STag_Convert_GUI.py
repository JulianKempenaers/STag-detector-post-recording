...
DEFAULT_SETTINGS = {
    ...
    "filename_addon": "NoReconstruction_A2",
...
class STag_Convert_GUI:
    ...
    def create_widgets(self):
        ...
        ttk.Label(frame, text="frame_reconstruction: Choose whether to run STAG detection on full frames or sparse frames.\nFull reconstruction takes longer. Sparse frames only save changed pixels.").grid(row=2, column=0, sticky="w", pady=(10, 0))
        self.vars["frame_reconstruction"] = ttk.BooleanVar(value=self.settings.get("frame_reconstruction", False))
        ttk.Checkbutton(frame, text="Frame Reconstruction", variable=self.vars["frame_reconstruction"], bootstyle="success,round-toggle").grid(row=3, column=0, sticky="w", pady=2)

        # n_cols (spinbox)
        ttk.Label(frame, text="Track how many recent tags?\nControls how many recently detected tags are tracked and color-coded. If set to 1: tags are not tracked and no color coding will occur.").grid(row=2, column=1, sticky="w", padx=10, pady=(10, 0))
        self.vars["n_cols"] = ttk.IntVar(value=self.settings.get("n_cols", 21))
        ttk.Spinbox(frame, from_=1, to=21, textvariable=self.vars["n_cols"], bootstyle="success", width=5).grid(row=3, column=1, sticky="w", padx=10, pady=2)
